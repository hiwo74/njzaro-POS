...
export function StatCard({ label, value, sublabel, icon, delay = 0, accent = false }: StatCardProps) {
  return (
    <motion.div
      ...
      className={`rounded-xl border p-4 ${accent ? "bg-brand-500 border-brand-500 text-white" : "bg-surface border-brand-100"}`}
    >
      <div className={`text-sm flex items-center gap-1.5 ${accent ? "text-white/70" : "text-brand-900"}`}>
