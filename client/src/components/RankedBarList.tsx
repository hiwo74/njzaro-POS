...
export function RankedBarList({ items, emptyLabel = "No data yet.", defaultColor = "#2a78d6" }: RankedBarListProps) {
  if (items.length === 0) {
    return <div className="text-sm text-brand-900 py-4">{emptyLabel}</div>;
  }
      ...
      {items.map((item, i) => (
        <div key={item.key}>
          <div className="flex items-baseline justify-between gap-2 mb-1">
            ...
            <span className="text-sm text-brand-900 flex-shrink-0 tabular-nums">{item.formattedValue}</span>
          </div>
