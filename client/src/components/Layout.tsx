...
import { BottleIcon, CartIcon, ChartIcon, MenuIcon, PackageIcon, PowerIcon, ReceiptIcon, SettingsIcon, UserIcon } from "./Icons";
...
export function Layout() {
  ...
  const ADMIN_MORE_LINKS = [
    ...
    { to: "/admin/decants", label: t.nav.decants, icon: <PackageIcon /> },
  ...
  return (
    <div className="min-h-screen bg-brand-50 flex flex-col">
      {/* Desktop header */}
      <header className="no-print hidden lg:flex sticky top-0 z-30 items-center justify-between px-4 py-3 bg-surface/80 backdrop-blur-xl border-b border-brand-900/5">
          ...
          <nav className="flex items-center gap-1 ms-4">
            ...
            {isAdmin && (
              <>
                ...
                <NavLink to="/admin/decants" className={desktopLinkClass}>
                  {t.nav.decants}
                </NavLink>
        ...
        <div className="flex items-center gap-3 text-sm">
          ...
          <span className="text-brand-900">
            {user?.name} <span className="text-brand-900">({user?.role === "ADMIN" ? t.nav.roleAdmin : t.nav.roleCashier})</span>
          </span>
          <button
      ...
      {/* Mobile header */}
      <header className="no-print lg:hidden sticky top-0 z-30 flex items-center justify-between px-4 py-3 pt-safe bg-surface/80 backdrop-blur-xl border-b border-brand-900/5">
      ...
      <nav className="no-print lg:hidden fixed bottom-0 inset-x-0 z-30 flex h-16 pb-safe bg-surface/85 backdrop-blur-xl border-t border-brand-900/5">
      ...
      <AnimatePresence>
        {moreOpen && (
          <>
            ...
            />
            <motion.div
              className="fixed bottom-0 inset-x-0 z-40 bg-surface rounded-t-2xl pb-safe shadow-xl"
              initial={{ y: "100%" }}
              animate={{ y: 0 }}
              exit={{ y: "100%" }}
