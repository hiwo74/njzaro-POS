...
function TrendTooltip({ active, payload, lang, t }: { active?: boolean; payload?: { payload: { date: string; revenueIqd: number; saleCount: number } }[]; lang: string; t: Translations }) {
  ...
  return (
    <div className="bg-surface border border-brand-100 rounded-lg shadow-md px-3 py-2 text-xs">
      <div className="font-medium text-brand-900">{label}</div>
      <div className="text-brand-900">{formatIqd(d.revenueIqd)}</div>
...
function CashierPerformance({ cashiers, t }: { cashiers: CashierStat[]; t: Translations }) {
  if (cashiers.length === 0) {
    return <div className="text-sm text-brand-900 py-2">{t.reports.noSalesThisMonth}</div>;
  }
      ...
      {cashiers.map((c) => (
        <div key={c.cashierId}>
          <div className="flex items-baseline justify-between gap-2 mb-1">
            ...
            <span className="text-sm text-brand-900 tabular-nums">{formatIqd(c.revenueIqd)}</span>
          </div>
...
export function ReportsPage() {
  ...
  if (!data) {
    return <div className="p-4 text-brand-900">{t.reports.loadingReports}</div>;
  }
  ...
  return (
    <div className="p-4 pb-8 space-y-5 max-w-6xl">
      <div className="flex items-center justify-between">
        ...
        <Link to="/admin/cash-up" className="press text-sm text-brand-900 underline">
      ...
      <motion.div
        ...
        className="bg-surface border border-brand-100 rounded-xl p-4"
      >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
        ...
        >
          ...
          {data.lowStock.length === 0 && <div className="text-sm text-brand-900 py-2">{t.reports.allGood}</div>}
