    ...
    <div className="min-h-screen bg-brand-50 flex flex-col items-center py-6 px-4 pt-safe pb-safe">
      <div className="no-print flex flex-wrap items-end gap-3 mb-4 w-full max-w-md">
        ...
        <button
          ...
          className="press px-4 py-2.5 bg-white border border-brand-100 text-brand-900 rounded-lg font-medium text-sm"
        >
      ...
      {loading || !data ? (
        <div className="text-brand-500">{t.cashUp.loading}</div>
      ) : (
        <motion.div
          initial={{ opacity: 0, scale: 0.97 }}
          animate={{ opacity: 1, scale: 1 }}
          transition={{ duration: 0.3, ease: [0.23, 1, 0.32, 1] }}
          className="bg-white shadow-lg rounded-xl p-6 w-full max-w-md font-mono text-sm"
        >
