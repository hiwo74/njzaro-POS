...
export function UsersPage() {
  ...
  return (
    <div className="p-4 pb-8">
      ...
      <form onSubmit={handleCreate} className="bg-white border border-brand-100 rounded-xl p-4 mb-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
      ...
      <div className="lg:hidden space-y-2">
        {users.map((u) => (
          <div key={u.id} className="bg-white rounded-xl border border-brand-100 p-3 flex items-center justify-between">
            <div>
            ...
            <button onClick={() => toggleActive(u)} className="press text-sm text-brand-600 font-medium">
          ...
          <tbody>
            {users.map((u) => (
              <tr key={u.id} className="border-b border-brand-50">
                ...
                <td className="p-3">
                  <button onClick={() => toggleActive(u)} className="press text-brand-600 underline">
                    {u.active ? t.users.deactivate : t.users.reactivate}
                  </button>
                </td>
              </tr>
