...
export function ProductsPage() {
  ...
  return (
    <div className="p-4 pb-8">
      ...
      {products.length === 0 && (
        <div className="bg-white rounded-xl border border-brand-100 p-8 text-center text-brand-500">{t.products.noProductsYet}</div>
      )}

      {/* Mobile: card list */}
      <div className="lg:hidden space-y-2">
        {products.map((p) => (
          <div key={p.id} className="bg-white rounded-xl border border-brand-100 p-3 flex gap-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between gap-2">
                <div className="min-w-0">
                  ...
                  <div className="text-xs text-brand-500">{p.brand} · {p.category} · {p.type}</div>
                </div>
                {!p.active && <span className="text-xs text-gray-400 flex-shrink-0">{t.common.inactive}</span>}
              </div>
              <div className="flex items-center justify-between mt-2">
                <span className="font-semibold text-brand-600">{formatIqd(p.sellPriceIqd)}</span>
              ...
              <div className="flex gap-3 mt-2 pt-2 border-t border-brand-50">
                <button onClick={() => startEdit(p)} className="press text-sm text-brand-600 font-medium">{t.common.edit}</button>
                {p.active && (
      ...
      {/* Desktop: table */}
      {products.length > 0 && (
        <div className="hidden lg:block bg-white rounded-xl border border-brand-100 overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="text-start text-brand-500 border-b border-brand-100">
                <th className="p-3"></th>
              ...
              {products.map((p) => (
                <tr key={p.id} className="border-b border-brand-50">
                  ...
                  <td className="p-3">
                    ...
                    <div className="text-xs text-brand-500">{p.brand}</div>
                  </td>
                  ...
                  <td className="p-3 flex gap-2">
                    <button onClick={() => startEdit(p)} className="press text-brand-600 underline">{t.common.edit}</button>
                    {p.active && (
