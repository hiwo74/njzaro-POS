...
export function SalesHistoryPage() {
  ...
  return (
    <div className="p-4 pb-8">
      ...
      <div className="bg-white border border-brand-100 rounded-xl p-3 mb-4 flex flex-wrap items-end gap-3">
        <div>
        ...
        {hasFilters && (
          <button onClick={clearFilters} className="press text-sm text-brand-500 underline pb-1.5">
      ...
      {sales.length === 0 && (
        <div className="bg-white rounded-xl border border-brand-100 p-8 text-center text-brand-500">
      ...
      <div className="lg:hidden space-y-2">
        {sales.map((s) => (
          <div key={s.id} className="bg-white rounded-xl border border-brand-100 p-3">
            ...
            <div className="flex items-center justify-between">
              <span className="text-sm text-brand-500">{t.salesHistory.itemsCount(s.items.reduce((n, i) => n + i.qty, 0))}</span>
            ...
            <div className="flex gap-3 mt-2 pt-2 border-t border-brand-50">
              <Link to={`/receipt/${s.id}`} className="press text-sm text-brand-600 font-medium">
              ...
              {sales.map((s) => (
                <tr key={s.id} className="border-b border-brand-50">
                  ...
                  <td className="p-3 flex gap-2">
                    <Link to={`/receipt/${s.id}`} className="press text-brand-600 underline">
                      {t.salesHistory.reprint}
                    </Link>
                    {user?.role === "ADMIN" && s.status === "COMPLETED" && (
