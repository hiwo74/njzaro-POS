  ...
  try {
    const sale = await db.$transaction(async (tx) => {
      ...
      for (const item of items) {
        ...
        // Decants are poured on demand from the original bottle — they have
        // no stock of their own, so never block or deduct on a decant sale.
        if (!product.isDecant && product.stock < item.qty) {
  ...
  try {
    const result = await db.$transaction(async (tx) => {
      ...
      for (const item of sale.items) {
        if (item.product?.isDecant) continue;
        await tx.product.update({
          where: { id: item.productId },
